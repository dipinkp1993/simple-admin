      ...
      @if($invoices->isEmpty())
            ...
            <div class="text-center">
             <x-action-message on="product-deleted" />
                <p class="text-xl font-bold">No invoice yet</p>
                <p class="text-sm">Let's create your first invoice</p>
                <x-button rose icon-right="plus" class="mt-6" href="{{route('invoices.create')}}" wire:navigate><i class="fas fa-plus mr-3"></i>Create
                    Invoice</x-button>
            </div>
      @else
                    <div class="flex justify-end">
                    <p class="text-xl pb-3 flex items-center">
                        <x-button rose icon-right="plus" class="mt-0" href="{{route('invoices.create')}}" wire:navigate> <i class="fas fa-plus mr-3"></i> Create
                    invoice</x-button>
                    </p>
                    </div>
                            ...
                            @foreach($invoices as $index=>$invoice)
                               <tr>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap">{{ ($invoices->currentPage()-1) * $invoices->perPage() + $loop->index + 1 }} </p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    ...
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap"> {{\Carbon\Carbon::parse(@$invoice->invoice_date)->format('d-M-Y')}}</p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap">
                                     ...
                                     <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap"> {{@$invoice->amount}}</p>
                                    </td>
                                     <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <p class="text-gray-900 whitespace-no-wrap"> {{ucwords(@$invoice->status)}}</p>
                                    </td>
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                     <x-button.circle rose  icon="pencil"
                                    href="{{route('invoices.edit',$invoice)}}" wire:navigate tooltip="Edit"></x-button.circle>
                                    </td>
